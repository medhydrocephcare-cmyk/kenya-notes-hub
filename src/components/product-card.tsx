...
const COURSE_TINT: Record<string, { bg: string; ring: string }> = {
  ...
  atd:  { bg: "from-sky-500 to-sky-700",          ring: "ring-sky-600/30" },
  cs:   { bg: "from-violet-500 to-violet-700",    ring: "ring-violet-600/30" },
  cifa: { bg: "from-amber-500 to-amber-700",      ring: "ring-amber-600/30" },
  ccp:  { bg: "from-rose-500 to-rose-700",        ring: "ring-rose-600/30" },
  cict: { bg: "from-cyan-500 to-cyan-700",        ring: "ring-cyan-600/30" },
  dcm:  { bg: "from-pink-500 to-pink-700",        ring: "ring-pink-600/30" },
  dict: { bg: "from-teal-500 to-teal-700",        ring: "ring-teal-600/30" },
  fab:  { bg: "from-lime-500 to-lime-700",        ring: "ring-lime-600/30" },
};
    ...
    <div className="group relative flex flex-col overflow-hidden rounded-2xl border border-border/60 bg-card transition hover:-translate-y-0.5 hover:border-brand/40 hover:shadow-card">
        ...
        <div className="relative flex h-full flex-col p-4 text-white">
          <div className="flex items-center gap-2">
            <span className="rounded-md bg-white/25 px-2 py-0.5 text-[10px] font-black uppercase tracking-wider backdrop-blur">
              {course?.code ?? paper.courseSlug.toUpperCase()}
            </span>
          ...
          <div className="mt-auto">
            ...
            <div className="mt-1.5 line-clamp-1 text-[10px] font-semibold uppercase tracking-wider text-white/85">
              {sitting} · KASNEB
            </div>
          </div>
        </div>
      ...
      <div className="flex flex-1 flex-col p-4">
        <div className="mb-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-[11px] text-muted-foreground">
          {paper.pages ? (
            <span className="inline-flex items-center gap-1"><FileText className="h-3 w-3" /> {paper.pages} pages</span>
          ) : null}
          {typeof paper.downloadCount === "number" && paper.downloadCount > 0 ? (
            <span className="inline-flex items-center gap-1"><Download className="h-3 w-3" /> {paper.downloadCount}</span>
          ) : null}
